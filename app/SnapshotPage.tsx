...
// Los eventos de cada jugador ya pedidos en esta sesión.
...
const apellido = (nombre: string) => nombre.trim().split(/\s+/).slice(-1)[0] ?? nombre;
...
function Tarjeta({ titulo, subtitulo, leyenda, children, clase = "" }: { titulo: string; subtitulo?: string; leyenda?: ReactNode; children: ReactNode; clase?: string }) {
...
function RadarFamilias({ valores }: { valores: GrupoCompuesto["valores"] extends Map<number, infer V> ? V : never }) {
...
export function SnapshotPage({ rows, indice, informe, perfilTm, minutosMin, destinatario = "", logoDestinatario = "", onAbrirJugador }: {
  ...
  useEffect(() => {
    ...
    if (guardado) { setEventos(guardado); setEstado(""); return; }
    ...
    fetchEventosJugador(liga, temporada, club, nombre)
      .then((respuesta) => {
        if (!vivo) return;
        memoria.set(clave, respuesta);
  ...
  const grupo = useMemo(() => familiasCompuestas(rows, informe.cohort, minutosMin, indice), [rows, informe.cohort, minutosMin, indice]);
  const propias = grupo.valores.get(indice) ?? {};
  const nombreGrupo = t(PERFILES.find((perfil) => perfil.id === informe.cohort)?.nombre ?? informe.cohort);
  ...
  const destacados = parecidos.slice(0, 5).map((p) => p.index);
  const nombres = (i: number) => String(rows[i]?.Player ?? "");

  const top = useMemo(() => grupo.indices
    .map((i) => ({ i, z: grupo.valores.get(i)?.[familiaTop]?.z ?? Number.NaN }))
    .filter((p) => Number.isFinite(p.z))
    .sort((a, b) => b.z - a.z)
    .slice(0, 10), [grupo, familiaTop]);

  // ---- Eventos, ya separados por mapa ----
  const lista: EventoSB[] = eventos?.eventos ?? [];
  const tiros = lista.filter((e) => e.t === "Shot" && e.l);
  const goles = tiros.filter((e) => e.o === "Goal").length;
  const xgTotal = tiros.reduce((s, e) => s + (e.xg ?? 0), 0);
  const ocasiones = lista.filter((e) => e.t === "Pass" && (e.sa || e.ga) && e.l && e.e);
  const asistencias = ocasiones.filter((e) => e.ga).length;
  const regates = lista.filter((e) => e.t === "Dribble" && e.l);
  const regatesBien = regates.filter((e) => e.o === "Complete").length;
  const pasesJuego = lista.filter((e) => esPaseEnJuego(e) && e.l);
  const acciones = lista.filter((e) => ["Pass", "Carry", "Ball Receipt*", "Dribble", "Shot"].includes(e.t) && esJuegoAbierto(e) && e.l);
  const defensivas = lista.filter((e) => ((e.t === "Duel" && e.dt === "Tackle") || ["Ball Recovery", "Interception", "Clearance", "Block"].includes(e.t)) && e.l);
  const cuenta = (tipo: string, dt?: string) => defensivas.filter((e) => e.t === tipo && (!dt || e.dt === dt)).length;
  const minutosPos = Object.entries(eventos?.posiciones ?? {}).filter(([, m]) => m > 0);
  const totalPos = minutosPos.reduce((s, [, m]) => s + m, 0);

  const claseOcasion = (e: EventoSB) => (e.ga ? "asistencia" : e.pt === "Corner" || e.pt === "Free Kick" ? "parado" : e.cr ? "centro" : "juego");
  const sinEventos = !eventos || !eventos.partidos;
  ...
  return <section className="snap-page">
    <header>
      <div>
        ...
        <h2>{jugador}</h2>
        <p>{tf("Familias contra {n} {grupo} de la base cargada, con al menos {m} minutos.", { n: grupo.indices.length, grupo: nombreGrupo.toLowerCase(), m: minutosMin })}</p>
      </div>
    </header>
    {estado && <p className="snap-estado" role="status">{estado}</p>}

    <div className="snap-fila snap-fila-1">
      <div className="snap-columna">
        <div className="snap-ficha">
        ...
        <ol className="snap-tabla" aria-label={tf("Familias frente a {n} {grupo}", { n: grupo.indices.length, grupo: nombreGrupo.toLowerCase() })}>
          <li className="snap-tabla-titulo">{tf("frente a {n} {grupo}", { n: grupo.indices.length, grupo: nombreGrupo.toLowerCase() })}</li>
      ...
      <Tarjeta titulo={t("Familias")} subtitulo={t("Largo: percentil · cifra: desviaciones típicas")} clase="snap-radar-tarjeta">
        <RadarFamilias valores={propias} />
      </Tarjeta>

      <Tarjeta titulo={t("Top 10")} clase="snap-top">
        <Desplegable etiqueta={t("Familia")} valor={t(COMPUESTAS.find((c) => c.id === familiaTop)!.etiqueta)} activo={false}>
          <select aria-label={t("Familia")} value={familiaTop} onChange={(evento) => setFamiliaTop(evento.target.value)}>
      ...
      <Tarjeta titulo={t("Tiempo en cada puesto")} subtitulo={eventos ? tf("{m} minutos en {n} puestos", { m: num(totalPos), n: minutosPos.length }) : undefined} clase="snap-puestos">
    ...
    <div className="snap-fila snap-fila-2">
      <Tarjeta titulo={tf("Los que más se parecen a {nombre}", { nombre: apellido(jugador) })} subtitulo={tf("{n} más cercanos · misma posición", { n: parecidos.length })} clase="snap-parecidos">
        <ol>
          {parecidos.map((p, posicion) => (
      ...
      <Tarjeta titulo={t("Tiros")} subtitulo={sinEventos ? undefined : tf("{n} tiros · {g} goles · {x} xG", { n: tiros.length, g: goles, x: num(xgTotal, 2) })}
        ...
        <Cancha mitad etiqueta={t("Tiros")}>
          {tiros.map((e, i) => { const [X, Y] = aVertical(e.l!); return <circle key={i} cx={X} cy={Y} r={0.9 + Math.sqrt(Math.max(e.xg ?? 0.01, 0.01)) * 5} className={e.o === "Goal" ? "snap-tiro gol" : "snap-tiro"} />; })}
        </Cancha>
      </Tarjeta>
      <Tarjeta titulo={t("Ocasiones creadas")} subtitulo={sinEventos ? undefined : tf("{n} pases a tiro · {a} asistencias", { n: ocasiones.length, a: asistencias })}
      ...
      <Tarjeta titulo={t("Regates")} subtitulo={sinEventos ? undefined : tf("{b} de {n} completados · {p}%", { b: regatesBien, n: regates.length, p: regates.length ? Math.round((regatesBien / regates.length) * 100) : 0 })}
    ...
    <div className="snap-fila snap-fila-3">
      <Tarjeta titulo={t("Dónde empieza sus pases")} subtitulo={sinEventos ? undefined : tf("{n} pases en juego abierto", { n: num(pasesJuego.length) })}>
        <RejillaCampo puntos={pasesJuego.map((e) => [e.l![0], e.l![1]] as Punto)} modo="pct" etiqueta={t("Dónde empieza sus pases")} />
      </Tarjeta>
      <Tarjeta titulo={t("Dónde terminan sus pases")} subtitulo={sinEventos ? undefined : tf("{n} destinos", { n: num(pasesJuego.filter((e) => e.e).length) })}>
        <RejillaCampo puntos={pasesJuego.filter((e) => e.e).map((e) => [e.e![0], e.e![1]] as Punto)} modo="pct" etiqueta={t("Dónde terminan sus pases")} />
      </Tarjeta>
      <Tarjeta titulo={t("Mapa de calor")} subtitulo={sinEventos ? undefined : tf("{n} acciones en juego abierto", { n: num(acciones.length) })}>
        {(() => {
          const { celdas, columnas, filas } = densidad(acciones.map((e) => [e.l![0], e.l![1]] as Punto));
          const w = 80 / columnas, h = 120 / filas;
          return <Cancha etiqueta={t("Mapa de calor")} fondo={celdas.map((fila, f) => fila.map((v, c) => v > 0.02
            ? <rect key={`${f}-${c}`} x={c * w} y={f * h} width={w + 0.1} height={h + 0.1} className="snap-calor" style={{ fillOpacity: v * 0.9 }} />
            : null))} />;
        })()}
      </Tarjeta>
      <Tarjeta titulo={t("Acciones defensivas")} subtitulo={sinEventos ? undefined : tf("{n} en total · {e} entradas · {r} recuperaciones · {i} intercepciones · {d} despejes", {
    ...
    <div className="snap-fila snap-fila-4">
      <Tarjeta titulo={tf("Frente a su grupo: {grupo}", { grupo: nombreGrupo.toLowerCase() })} subtitulo={t("Cada punto es un jugador; relleno, él; con aro, sus cinco más parecidos")} clase="snap-enjambres">
      ...
      <Tarjeta titulo={t("Construcción frente a asociación")} subtitulo={t("Desviaciones típicas frente a su grupo")} clase="snap-dispersion-tarjeta">
        <Dispersion grupo={grupo} objetivo={indice} destacados={destacados} nombres={nombres} ejeX="construccion" ejeY="asociacion" />
      </Tarjeta>
    </div>

    <PieDeReporte asunto={jugador} destinatario={destinatario} logo={logoDestinatario} />
  </section>;
}
