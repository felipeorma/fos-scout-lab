...
import { ChevronRight, FileSpreadsheet, Merge, MoreHorizontal, Plus, Sparkles, Trash, Upload } from "./Icons";
import { Interruptor } from "./Interruptor";
import { FilaDeMenu, MenuFlotante } from "./MenuFlotante";
...
export function DatosPage({
  ...
  const proveedorDe = (archivo: string) => (/^SkillCorner/i.test(archivo) ? "skillcorner"
    : /^StatsBomb/i.test(archivo) ? "statsbomb" : "wyscout") as "wyscout" | "statsbomb" | "skillcorner";
  ...
  return <section className="datos-page">
    ...
    {rows.length > 0 ? <>
      {/* El resumen, en una tarjeta: las tres cifras con separadores y las
          plataformas al pie. Las cifras iban en el color del acento, igual que
          un botón, y no son algo que se pulse. */}
      ...
      <section className="datos-seccion datos-anios">
        <h3 className="datos-lista-titulo">{t("Años")}</h3>
      ...
      {/* Lo cargado, como una lista de Ajustes: logos, nombre y temporada a la
          izquierda; el interruptor, al final de la fila, donde iOS los pone.
          Era una tabla con "Quitar" en la última columna, un botón destructivo
          a un dedo del interruptor. Quitar pasa a un menú: se llega igual,
          pero no se pulsa sin querer. */}
      <section className="datos-seccion datos-lista">
        <h3 className="datos-lista-titulo">{tf("Lo que hay cargado · {nombre}", { nombre })}</h3>
        <ul className="datos-competiciones">
          {competiciones.map((c) => {
            const encendida = !c.archivos.every((archivo) => apagadas.includes(archivo));
            return <li key={`${c.liga}-${c.anio}`} className={encendida ? "" : "apagada"}>
              <span className="datos-origen">
                {c.archivos.map((archivo) => <LogoPlataforma key={archivo} plataforma={proveedorDe(archivo)} alto={14} />)}
              </span>
              <span className="datos-competicion">
                <b>{c.liga}</b>
                <small>{[c.anio || null, [...new Set(c.archivos.map((archivo) => METRIC_SOURCE_COLORS[proveedorDe(archivo)].label))].join(" + ")].filter(Boolean).join(" · ")}</small>
              </span>
              ...
              />}
              {onQuitarCompeticion && competiciones.length > 1 && <MenuFlotante
                className="datos-mas"
                etiqueta={tf("Más opciones de {liga}", { liga: c.liga })}
                icono={<MoreHorizontal size={16} />}>
                <FilaDeMenu peligro onClick={() => onQuitarCompeticion(c.archivos)}>
                  <Trash size={14} /><span>{t("Quitar de la base")}</span>
                </FilaDeMenu>
              </MenuFlotante>}
            </li>;
          })}
    ...
    {/* Las tres formas de meter datos, como filas de una lista agrupada. Eran
        tres tarjetas grandes en rejilla, con el aspecto de una portada, en una
        pantalla que es de ajustes. La recomendada lleva la baldosa del acento. */}
    <section className="datos-seccion">
      <h3 className="datos-lista-titulo">{t("Añadir datos")}</h3>
      <div className="datos-acciones">
        ...
        <button type="button" className="datos-accion" disabled={cargando} onClick={onConectarApi}>
          <span className="datos-accion-icono"><Merge size={17} /></span>
          <span className="datos-accion-texto">
