...
const THEMES = [
  { name: "Bosque", background: "#e8eeeb", paper: "#ffffff", accent: "#1f7666", textColor: "#17323a" },
  { name: "Noche", background: "#dde3e5", paper: "#102b36", accent: "#58bca8", textColor: "#f3f7f6" },
  { name: "Arena", background: "#eee9df", paper: "#fffdf8", accent: "#c76a40", textColor: "#2d3437" },
  { name: "Editorial", background: "#e9e7e5", paper: "#ffffff", accent: "#a93545", textColor: "#20262a" },
  { name: "Azul", background: "#e6edf2", paper: "#fdfefe", accent: "#286b92", textColor: "#182e3b" },
];

function imageBlock(id: string, title: string, span = 1, height = 280): PageBlock {
  return { id, type: "image", title, content: "", image: "", span, height, fit: "contain", bold: false, italic: false, font: "barlow", fontSize: 18, color: "#17323a", align: "left" };
}

function textBlock(id: string, title: string, content: string, span = 2, height = 190): PageBlock {
  return { id, type: "text", title, content, image: "", span, height, fit: "cover", bold: false, italic: false, font: "barlow", fontSize: 17, color: "#2c4249", align: "left" };
}
...
export function ReportPageDesigner({ pageNumber, player, team, position }: { pageNumber: 2 | 3; player: string; team: string; position: string }) {
  ...
  function addBlock(type: BlockType) {
    ...
    const block = type === "image"
      ...
      : textBlock(id, "Nuevo bloque de texto", "Escribe aquí tu análisis…", Math.min(2, config.columns), 180);
  ...
  const canvasStyle = {
    "--designer-paper": config.paper,
    "--designer-accent": config.accent,
    "--designer-text": config.textColor,
  ...
  return (
    <div className="designer-workspace">
      <aside className="designer-controls">
        ...
        <div className="designer-section">
          <div className="designer-section-title"><span>Color de diseño</span><Palette size={15} /></div>
          <div className="theme-swatches">{THEMES.map((theme) => <button key={theme.name} className={config.accent === theme.accent ? "active" : ""} onClick={() => applyTheme(theme)} title={theme.name}><i style={{ background: theme.paper }}><b style={{ background: theme.accent }} /></i><span>{theme.name}</span></button>)}</div>
          <div className="color-inputs"><label><span>Acento</span><input type="color" value={config.accent} onChange={(event) => setConfig((current) => ({ ...current, accent: event.target.value }))} /></label><label><span>Fondo</span><input type="color" value={config.paper} onChange={(event) => setConfig((current) => ({ ...current, paper: event.target.value }))} /></label></div>
        </div>
        ...
        <button className="reset-design" onClick={() => { const defaults = defaultPages()[pageNumber]; setConfig(() => defaults); setSelectedId(defaults.blocks[0].id); }}><RotateCcw size={14} /> Restablecer página</button>
      </aside>

      <section className="designer-stage" style={{ background: config.background }}>
        ...
        <article className="visual-report-page" style={canvasStyle}>
          <header className="visual-page-header">
            <div><span>FOS SCOUT LAB · INFORME VISUAL</span><h2>{player}</h2><p>{team} · {position}</p></div>
            <div><small>PÁGINA</small><b>0{pageNumber}</b></div>
          </header>
          ...
          <div className="visual-block-grid" style={{ gridTemplateColumns: `repeat(${config.columns}, minmax(0, 1fr))`, gap: config.gap }} onDragOver={(event) => event.preventDefault()}>
            {config.blocks.map((block) => {
              ...
              const textStyle = { color: block.color, fontFamily: fontFamily(block.font), fontSize: block.fontSize, fontWeight: block.bold ? 700 : 400, fontStyle: block.italic ? "italic" : "normal", textAlign: block.align } as CSSProperties;
